...
def fetch_deals():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # 🚨 디버깅용: 뽐뿌에서 서버가 받아온 실제 HTML 앞부분 출력
            st.warning("⚠️ [디버깅] 뽐뿌 응답 HTML 미리보기")
            st.code(html[:500]) 
    ...
    try:
        ...
        if res.status_code == 200:
            # 🚨 디버깅용: 퀘이사존에서 서버가 받아온 실제 HTML 앞부분 출력
            st.warning("⚠️ [디버깅] 퀘이사존 응답 HTML 미리보기")
...
with st.spinner('핫딜 정보를 불러오는 중입니다... (디버깅 모드)'):
    data = fetch_deals()
...
if data:
    ...
    st.error("현재 올라온 당일 게시글이 없거나 데이터를 불러오지 못했습니다. 화면 상단의 [디버깅] 박스 내용을 확인해 주세요.")
